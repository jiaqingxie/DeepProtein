...
DeepProtein 2.0 removes DGL from the core installation path. Phases II and III add torch-geometric support for both single-protein and pair/PPI graph encoders through the `PyG_*` family, while legacy `DGL_*`, `PAGTN`, `EGT`, and `Graphormer` paths remain unsupported. Phase V further enables optional Laplacian positional encoding on the `PyG_*` path via ``compute_pos_enc=True``. Phase VI adds the first PyG-native graph transformer entry point via ``PyG_GraphGPS`` / ``GPSConv``; ``Exphormer`` and ``GRIT`` remain future work.
