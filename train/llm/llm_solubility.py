...
if __name__ == "__main__":
    ...
    train_solu = Solubility(path + '/DeepProtein/data', 'train')
    valid_solu = Solubility(path + '/DeepProtein/data', 'valid')
    test_solu = Solubility(path + '/DeepProtein/data', 'test')

    if target_encoding in ['DGL_GAT', 'DGL_GCN', 'DGL_NeuralFP', 'DGL_AttentiveFP', 'DGL_MPNN', 'PAGTN', 'EGT', 'Graphormer']:
        train_protein_processed, train_target, train_protein_idx = collate_fn(train_solu, graph=True)
        valid_protein_processed, valid_target, valid_protein_idx = collate_fn(valid_solu, graph=True)
        test_protein_processed, test_target, test_protein_idx = collate_fn(test_solu, graph=True)

    else:
        train_protein_processed, train_target, train_protein_idx = collate_fn(train_solu)
        valid_protein_processed, valid_target, valid_protein_idx = collate_fn(valid_solu)
        test_protein_processed, test_target, test_protein_idx = collate_fn(test_solu)
    ...
    model.LLM_test_and_log(test_protein_processed, test_target, "solubility", False)
