...
from DeepPurpose_PP.utils import *
from DeepPurpose_PP.model_helper import Encoder_MultipleLayers, Embeddings
from DeepPurpose_PP.encoders import *
...
class Protein_Prediction:
    ...
    def test_(self, data_generator, model, repurposing_mode=False, test=False, verbose=True):
        ...
        for i, (v_p, label) in enumerate(data_generator):
            if self.target_encoding in ['Transformer', 'DGL_GCN', 'DGL_GAT', 'DGL_NeuralFP', 'DGL_AttentiveFP', 'DGL_MPNN']:
                v_p = v_p
            else:
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        if self.target_encoding in ['DGL_GCN', 'DGL_GAT', 'DGL_NeuralFP', 'DGL_AttentiveFP', 'DGL_MPNN']:
        ...
        for epo in range(train_epoch):
            ...
            for i, (v_p, label) in enumerate(training_generator):
                ...
                if self.binary:
                    ...
                    if self.target_encoding not in ['DGL_GCN', 'DGL_GAT', 'DGL_NeuralFP', 'DGL_AttentiveFP', 'DGL_MPNN']:
                        label = torch.squeeze(label, 1)
                    loss = loss_fct(n, label)
